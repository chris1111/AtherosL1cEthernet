...
/* Added for Mac driver. */

/* Buffer address mask, 32 bit addressable and page aligned. */
// #define AT_BUF_ADDR_MASK	(0x00000000ffffffffULL & PAGE_MASK)
/* Buffer address mask, 32 bit addressable and 8-byte boundry aligned. */
#define AT_BUF_ADDR_MASK	(0x00000000ffffffffULL & ~(sizeof(u_int64_t) - 1))


#define PHY_AUTO_NEG_TIME             45	/* 4.5 Seconds */
#define PHY_FORCE_TIME                20	/* 2.0 Seconds */

#define AT_RX_BUFF_COUNT              512
#define AT_TX_BUFF_COUNT              1024

#define AT_TRY_LINK_TIMEOUT           (4 * HZ)
...
struct atl1c_hw
{
  ...
  u8 preamble_len;
  u16 max_frame_size; // The maximum ethernet packet data size, without the
                      // header, CRC or VLAN tag added on. Generally 1500
                      // for non-jumbo frames.
  u16 min_frame_size;
...
do                                                                             \
{                                                                              \
  if ((a)->nic_type == athr_l2c_b2 && (reg) < 0x1400)                          \
  {                                                                            \
		readl((a)->hw_addr + reg);                                                 \
  }                                                                            \

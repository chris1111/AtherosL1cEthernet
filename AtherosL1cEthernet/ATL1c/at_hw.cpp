 ...
 */
int
atl1c_check_eeprom_exist (atl1c_hw * hw)
{
  u32 data;
...
void
atl1c_hw_set_mac_addr (struct atl1c_hw *hw, u8 * mac_addr)
{
  u32 value;
  /*
   * 00-0B-6A-F6-00-DC
   ...
   */
  value = mac_addr[2] << 24 |
  mac_addr[3] << 16 | mac_addr[4] << 8 | mac_addr[5];
...
static bool
atl1c_read_current_addr (struct atl1c_hw *hw, u8 * eth_addr)
{
  u32 addr[2];
 ...
 */
static int
atl1c_get_permanent_address (struct atl1c_hw *hw)
{
  u32 i;
  u32 otp_ctrl_data;
  u32 twsi_ctrl_data;
...
bool
atl1c_read_eeprom (struct atl1c_hw * hw, u32 offset, u32 * p_value)
{
  int i;
  int ret = false;
  u32 otp_ctrl_data;
  ...
  AT_READ_REG (hw, REG_OTP_CTRL, &otp_ctrl_data);
  if (!(otp_ctrl_data & OTP_CTRL_CLK_EN))
    AT_WRITE_REG (hw, REG_OTP_CTRL, (otp_ctrl_data | OTP_CTRL_CLK_EN));
 ...
 */
int
atl1c_read_mac_addr (struct atl1c_hw *hw)
{
  int err = 0;
 ...
 */
u32
atl1c_hash_mc_addr (atl1c_hw * hw, u8 * mc_addr)
{
  u32 crc32;
  u32 value = 0;
  int i;
 ...
 */
void
atl1c_hash_set (atl1c_hw * hw, u32 hash_value)
{
 ...
 */
bool
atl1c_wait_mdio_idle (struct atl1c_hw *hw)
{
  u32 val;
  int i;
...
void
atl1c_stop_phy_polling (struct atl1c_hw *hw)
{
...
void
atl1c_start_phy_polling (struct atl1c_hw *hw, u16 clk_sel)
{
  u32 val;
 ...
 */
int
atl1c_read_phy_core (struct atl1c_hw *hw, bool ext, u8 dev,
                     u16 reg, u16 * phy_data)
{
  ...
  *phy_data = 0;
  ...
  if (ext)
  {
    ...
    val = MDIO_CTRL_SPRES_PRMBL |
    FIELDX (MDIO_CTRL_CLK_SEL, clk_sel) |
    MDIO_CTRL_START | MDIO_CTRL_MODE_EXT | MDIO_CTRL_OP_READ;
  }
  else
  {
    val = MDIO_CTRL_SPRES_PRMBL |
    FIELDX (MDIO_CTRL_CLK_SEL, clk_sel) |
    FIELDX (MDIO_CTRL_REG, reg) | MDIO_CTRL_START | MDIO_CTRL_OP_READ;
  }
  AT_WRITE_REG (hw, REG_MDIO_CTRL, val);
 ...
 */
int
atl1c_write_phy_core (struct atl1c_hw *hw, bool ext, u8 dev,
                      u16 reg, u16 phy_data)
{
  u32 val;
  u16 clk_sel = MDIO_CTRL_CLK_25_4;
  ...
  if (ext)
  {
    ...
    MDIO_CTRL_START | MDIO_CTRL_MODE_EXT;
  }
  else
  {
    val = MDIO_CTRL_SPRES_PRMBL |
    FIELDX (MDIO_CTRL_CLK_SEL, clk_sel) |
    FIELDX (MDIO_CTRL_DATA, phy_data) |
    FIELDX (MDIO_CTRL_REG, reg) | MDIO_CTRL_START;
  }
  AT_WRITE_REG (hw, REG_MDIO_CTRL, val);
 ...
 */
int
atl1c_read_phy_reg (struct atl1c_hw *hw, u16 reg_addr, u16 * phy_data)
{
 ...
 */
int
atl1c_write_phy_reg (struct atl1c_hw *hw, u32 reg_addr, u16 phy_data)
{
...
int
atl1c_read_phy_ext (struct atl1c_hw *hw, u8 dev_addr,
...
int
atl1c_write_phy_ext (struct atl1c_hw *hw, u8 dev_addr,
                     u16 reg_addr, u16 phy_data)
{
...
int
atl1c_read_phy_dbg (struct atl1c_hw *hw, u16 reg_addr, u16 * phy_data)
{
  int err;
...
int
atl1c_write_phy_dbg (struct atl1c_hw *hw, u16 reg_addr, u16 phy_data)
{
  int err;
 ...
 */
static int
atl1c_phy_setup_adv (struct atl1c_hw *hw)
{
  ...
  if (hw->link_cap_flags & ATL1C_LINK_CAP_1000M)
  {
    ...
    if (hw->autoneg_advertised & ADVERTISED_Autoneg)
      mii_giga_ctrl_data |= ADVERTISE_1000HALF | ADVERTISE_1000FULL;
  }
...
void
atl1c_phy_disable (struct atl1c_hw *hw)
{
  atl1c_power_saving (hw, 0);
}

int
atl1c_phy_reset (struct atl1c_hw *hw)
{
  DbgPrint (4, "atl1c_phy_reset()\n");
  ...
  phy_ctrl_data &= ~(GPHY_CTRL_EXT_RESET | GPHY_CTRL_PHY_IDDQ |
                     GPHY_CTRL_GATE_25M_EN | GPHY_CTRL_PWDOWN_HW |
                     GPHY_CTRL_CLS);
  ...
  if (!(hw->ctrl_flags & ATL1C_HIB_DISABLE))
  {
    ...
    /* hib */
    atl1c_write_phy_dbg (hw, MIIDBG_SYSMODCTRL, SYSMODCTRL_IECHOADJ_DEF);
  }
  else
  {
    /* disable pws */
    atl1c_read_phy_dbg (hw, MIIDBG_LEGCYPS, &phy_data);
    atl1c_write_phy_dbg (hw, MIIDBG_LEGCYPS, phy_data & ~LEGCYPS_EN);
    /* disable hibernate */
    atl1c_read_phy_dbg (hw, MIIDBG_HIBNEG, &phy_data);
    atl1c_write_phy_dbg (hw, MIIDBG_HIBNEG, phy_data & HIBNEG_PSHIB_EN);
  }
      ...
      hw->nic_type == athr_l2c_b2)
  {
    ...
    atl1c_write_phy_ext (hw, MIIEXT_PCS, MIIEXT_CLDCTRL3, L2CB_CLDCTRL3);
  }
  ...
  if (err)
  {
    ErrPrint ("Cannot enable PHY interrupt!\n");
    return err;
  }
  return 0;
}
...
int
atl1c_phy_init (struct atl1c_hw *hw)
{
  DbgPrint (4, "atl1c_phy_init()\n");
  ...
  switch (hw->media_type)
  {
    case MEDIA_TYPE_AUTO_SENSOR:
      ...
      if (ret_val)
      {
        ErrPrint ("Cannot setup auto negotiation!\n");
        return ret_val;
      }
...
int
atl1c_phy_to_ps_link (struct atl1c_hw *hw)
{
  DbgPrint (4, "atl1c_phy_to_ps_link()\n");
  ...
  if (phy_data & BMSR_LSTATUS)
  {
    ...
    if (mii_lpa_data)
    {
      for (i = 0; i < AT_SUSPEND_LINK_TIMEOUT; i++)
      {
        ...
        if (phy_data & BMSR_LSTATUS)
        {
          if (atl1c_get_speed_and_duplex (hw, &speed, &duplex) != 0)
            DbgPrint (1, "Get speed and duplex failed.\n");
          break;
        }
      }
    }
...
int
atl1c_restart_autoneg (struct atl1c_hw *hw)
{
  int err = 0;
  u16 mii_bmcr_data = BMCR_RESET;
...
int
atl1c_power_saving (struct atl1c_hw *hw, u32 wufc)
{
  DbgPrint (4, "atl1c_power_saving()\n");
  ...
  phy_ctrl |= GPHY_CTRL_SEL_ANA_RST | GPHY_CTRL_HIB_PULSE | GPHY_CTRL_HIB_EN;
  if (!wufc)
  {				/* without WoL */
    master_ctrl |= MASTER_CTRL_CLK_SEL_DIS;
  ...
  if (wufc & AT_WUFC_LNKC)
  {
    ...
    {
      DbgPrint (1, "%s: Write PHY MII_IER failed.\n", atl1c_driver_name);
    }
  }
  /* clear PHY interrupt */
...
atl1c_post_phy_linkchg (struct atl1c_hw *hw, u16 link_speed)
{
  ...
  {				/* link up */
    ...
    {
      atl1c_read_phy_ext (hw, MIIEXT_PCS, MIIEXT_CLDCTRL6, &phy_val);
